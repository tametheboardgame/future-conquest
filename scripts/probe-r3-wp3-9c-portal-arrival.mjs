...
async function waitForArrivalEvidence(page, timeout = 15000) {
...
async function triggerFreshCampaignArrival(page) {
  ...
  await page.locator('.command-map-workspace').waitFor({ state: 'visible' });
  await waitForArrivalEvidence(page);
}
...
async function validateOpeningAndMaterialisation(page) {
  await page.waitForFunction(() => window.__r3PortalArrival?.phase === 'opening'
    ...
    && window.__r3FormationMiniatures.pieces.every(piece => piece.visible === false), null, { timeout: 2500 });
  ...
  if (opening.formationCount === 0 || opening.formationCount !== opening.rendererFormationCount) {
    throw new Error(`arrival formation count mismatch (${opening.formationCount}/${opening.rendererFormationCount})`);
  }
...
try {
  ...
  // Warm the physical renderer first. A fresh campaign legitimately rebuilds its
  // formation layer, and the portal must wait behind the opaque arrival veil until
  // the new authoritative formation targets exist.
  ...
  if (await page.evaluate(() => sessionStorage.getItem('future-conquest:r3-wp39c-arrival-played')) !== 'true') {
    throw new Error('arrival presentation marker was not consumed');
  }
  ...
  if (await reducedPage.locator('.r3-portal-arrival').count()) {
    await reducedPage.locator('.r3-portal-arrival').waitFor({ state: 'detached', timeout: 15000 });
  }
  ...
  await reducedPage.locator('.r3-portal-arrival').waitFor({ state: 'detached', timeout: 3000 });
  const reducedElapsedMs = Date.now() - reducedStarted;
  if (reducedElapsedMs > 15000) throw new Error(`reduced-motion arrival remained active too long (${reducedElapsedMs}ms)`);
