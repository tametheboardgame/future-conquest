...
const overridePath = path.resolve(process.env.MAP_ASSIGNMENT_OVERRIDES || 'data/authored/map-region-overrides.json');
...
const easternSourceCodes = new Set(['BY', 'MD', 'UA', 'RU']);
const easternIso = { BY: 'BLR', MD: 'MDA', UA: 'UKR', RU: 'RUS' };
function readEasternAdmin(countryCode) {
  const iso = easternIso[countryCode];
  const filename = path.join(sourceRoot, 'geoboundaries', `${iso}-ADM1.geojson`);
  const collection = JSON.parse(fs.readFileSync(filename, 'utf8'));
  return collection.features.map(feature => turf.feature(feature.geometry, {
    id: feature.properties.shapeID,
    na: feature.properties.shapeName
  }));
}
...
function sourceRegionsFor(countryCode) {
  if (easternSourceCodes.has(countryCode)) {
    let regions = readEasternAdmin(countryCode);
    if (countryCode === 'RU') {
      const excluded = new Set(['Chukotka Autonomous Okrug']);
      regions = regions.filter(region => !excluded.has(region.properties.na) && turf.centroid(region).geometry.coordinates[0] <= 60.75);
    }
    return { regions, sourceYear: 2025, sourceLevel: 1, sourceName: 'geoBoundaries ADM1' };
  }
  ...
  return { regions, sourceYear, sourceLevel, sourceName: 'NUTS' };
}
...
for (const [countryCode, countryRows] of rowGroups) {
  const { regions, sourceYear, sourceLevel, sourceName } = sourceRegionsFor(countryCode);
  if (!regions.length) {
  ...
  for (const region of regions) {
    ...
    const overrideId = overrides[region.properties.id];
    const territory = overrideId ? countryRows.find(row => row.territory_id === overrideId) : ranked[0].row;
    if (!territory) throw new Error(`Invalid override for ${region.properties.id}: ${overrideId}`);
    ...
    mappings.push({
      ...
      assignment: overrideId ? 'manual-override' : 'nearest-strategic-centre-draft'
    });
  }
  ...
  for (const row of countryRows) {
    ...
    outputFeatures.push(turf.feature(dissolved.geometry, {
      ...
      admin_source: sourceName === 'NUTS' ? `NUTS ${sourceYear} level ${sourceLevel}` : `${sourceName} (release ${sourceYear})`,
...
for (let index = 0; index < outputFeatures.length; index += 1) {
  ...
  outputFeatures[index].properties.label_anchor = centreInside
    ? outputFeatures[index].properties.centre
    : turf.pointOnFeature(outputFeatures[index]).geometry.coordinates;
  outputFeatures[index].properties.label_anchor_adjusted = !centreInside;
}

const collection = turf.featureCollection(outputFeatures);
fs.mkdirSync(outputRoot, { recursive: true });
fs.writeFileSync(path.join(outputRoot, 'territories-standard-admin-comparison-v0.3.geojson'), `${JSON.stringify(collection)}\n`);
...
const mappingCsv = [mappingHeader.join(','), ...mappings
  ...
  .join('\n');
fs.writeFileSync(path.join(outputRoot, 'territory-source-region-mapping-v0.3.csv'), `${mappingCsv}\n`);
...
const labels = outputFeatures.map(item => {
  const point = projection(item.properties.label_anchor);
  if (!point) return '';
...
<text x="90" y="110" class="sub">Administrative-boundary comparison • Draft 0.3 • Approved 101-territory structure retained</text>
<g>${paths}</g><g>${labels}</g>
<g transform="translate(90 1715)"><text y="16" class="note">NUTS and geoBoundaries regions are assigned to approved strategic territories; explicit overrides preserve intended strategic groupings.</text><text y="43" class="note">Geometry is separate from claimant and controller data, including Crimea and other disputed areas.</text></g>
</svg>\n`;
fs.writeFileSync(path.join(outputRoot, 'standard-territories-admin-comparison-v0.3.svg'), svg);
...
const report = {
  ...
  warnings: [
    ...
    'Belarus, Moldova, Ukraine and Russia use geoBoundaries ADM1 geometry.',
    'Political claims and actual control remain outside geometry.'
  ]
};
fs.writeFileSync(path.join(outputRoot, 'admin-comparison-build-report-v0.3.json'), `${JSON.stringify(report, null, 2)}\n`);
