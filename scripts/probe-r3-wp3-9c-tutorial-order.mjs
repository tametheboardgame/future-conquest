...
async function waitForPortalWithoutTutorial(page, timeout = 60000) {
...
try {
  ...
  const duringArrival = await waitForPortalWithoutTutorial(page);
  ...
  const secondCampaign = await waitForPortalWithoutTutorial(page);
  await page.locator('.r3-portal-arrival').waitFor({ state: 'detached', timeout: 15000 });
