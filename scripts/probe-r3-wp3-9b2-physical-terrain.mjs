...
function containsColours(expression) {
...
try {
  ...
  if (paint.physical?.status !== 'ready' || paint.physical?.profileId !== 'physical-colour-v1') {
    throw new Error(`physical terrain runtime evidence missing: ${JSON.stringify(paint.physical)}`);
  }
  if (paint.hostPhysical !== 'physical-colour-v1') throw new Error(`host physical terrain marker missing: ${paint.hostPhysical}`);
  if (!paint.sourceLoaded || paint.layerType !== 'raster') throw new Error(`physical raster not loaded: ${JSON.stringify(paint)}`);
  if (paint.rasterOpacity !== 0.98 || paint.rasterSaturation !== 0.08 || paint.rasterContrast !== 0.06) {
    throw new Error(`physical raster paint mismatch: ${JSON.stringify(paint)}`);
  }
  if (paint.hillshadeExaggeration !== 0.4) throw new Error(`hillshade intensity mismatch: ${paint.hillshadeExaggeration}`);
  if (paint.territoryFillOpacity !== 0 || paint.stateWashOpacity !== 0 || paint.administrativeBorderOpacity !== 0) {
    throw new Error(`political wash regression: ${JSON.stringify(paint)}`);
  }
  if (!containsColours(paint.controlBorderColour) || !containsColours(paint.controlGlowColour)) {
    throw new Error(`controller border colours regressed: ${JSON.stringify(paint)}`);
  }
