...
test('all Pass 1 city definitions retain authored runtime assets as the registry expands',()=>{for(const [file,id] of [['london-selected.gltf','wp3.8a-v2-london-selected'],['paris-selected.gltf','wp3.8a-v2-paris-selected'],['brussels-selected.gltf','wp3.8a-v2-brussels-selected']]){assert.match(assets,new RegExp(id.replaceAll('.','\\.')));assert.match(assets,new RegExp(`selectedUrl: assetUrl\\('wp3-8a', '${file.replace('.','\\.')}\\'\\)`))}assert.doesNotMatch(assets,/nodeId: 'N-LONDON'[\s\S]*?rollout: 'authoring'/);assert.doesNotMatch(assets,/nodeId: 'N-PARIS'[\s\S]*?rollout: 'authoring'/);assert.doesNotMatch(assets,/nodeId: 'N-BRUSSELS'[\s\S]*?rollout: 'authoring'/)});
