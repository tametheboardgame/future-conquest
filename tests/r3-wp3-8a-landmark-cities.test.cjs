...
test('Selected view lazy-loads authored glTF with procedural distant/loading/error fallback',()=>{assert.match(layer,/import\('three\/examples\/jsm\/loaders\/GLTFLoader\.js'\)/);assert.match(layer,/loader\.loadAsync\(asset\.selectedUrl\)/);assert.match(layer,/rootVisible && lod === 'selected' && piece\.asset/);assert.match(layer,/piece\.fallbackRoot\.visible = rootVisible && !useAuthoredAsset/);assert.match(layer,/piece\.assetRoot\.visible = useAuthoredAsset/);assert.match(layer,/retaining procedural fallback/)});
