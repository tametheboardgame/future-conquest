export type LandmarkMiniatureCityVariant = 'london' | 'paris' | 'brussels' | 'amsterdam' | 'frankfurt' | 'bern';
export type LandmarkMiniatureRollout = 'runtime' | 'authoring';

export interface LandmarkMiniatureAssetDefinition {
  nodeId: 'N-LONDON' | 'N-PARIS' | 'N-BRUSSELS' | 'N-AMSTERDAM' | 'N-FRANKFURT' | 'N-BERN';
...
const assetUrl = (pass: 'wp3-8a' | 'wp3-8b', filename: string) => `${import.meta.env.BASE_URL}miniatures/${pass}/${filename}`;
